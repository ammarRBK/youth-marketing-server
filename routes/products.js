...
router.post('/addProduct',uploadFile.single('productImage'),cors(corsOptions),async (req,res)=>{
  ...
  let Production= {
    ...
    productDate: req.body.productDate != '' ? moment(req.body.productDate).format('YYYY-MM-DD') : null,
    expirationDate: req.body.expirationDate != '' ? moment(req.body.expirationDate).format('YYYY-MM-DD'): null,
